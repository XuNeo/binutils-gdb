...
core_target_open (const char *arg, int from_tty)
{
  ...
  if (ctx.valid ())
    ...
    {
      gdb::unique_xmalloc_ptr<char> failing_command;
	const char *command = bfd_core_file_failing_command
      (current_program_space->core_bfd ());
      if (command != nullptr && (failing_command = make_unique_xstrdup(command)) != nullptr)
